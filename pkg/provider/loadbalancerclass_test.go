...
func TestSyncLoadBalancerIfNeededWithMultipleIpUse(t *testing.T) {
	testCases := []struct {
		...
		{
			desc:              "another service who wants to use the same port, get a new IP address",
		...
		{
			desc:             "another service who wants to use the same port, but ip are used up, no address assigned",
